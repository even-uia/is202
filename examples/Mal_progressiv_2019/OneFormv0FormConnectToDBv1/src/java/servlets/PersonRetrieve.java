...
public class PersonRetrieve extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
           ...
           else if (hent.contains("Straight"))
           {
            ...
            checkDatabase(out,conn);
            readStudent(out, conn);
            out.println("</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    ...
    public void checkDatabase(PrintWriter out, Connection conn)
    {
        Statement stmt;
    ...
    public void readStudent(PrintWriter out, Connection conn)
    {
        Statement stmt;
        ...
        try {
            ...
            // Step 4: Process the ResultSet by scrolling the cursor forward via next().
            //  For each row, retrieve the contents of the cells with getXxx(columnName).
            out.println("The records selected from table STUDENT are:" +"<br>");
            int rowCount = 0;
            while(rset.next()) {   // Move the cursor to the next row, return false if no more row
                String snr = rset.getString("snr");
                String firstName = rset.getString("firstName");
                String lastName = rset.getString("lastName");
                out.println(rowCount +": " + firstName +"<br>");
                ++rowCount;
             }  // end while
             out.println("Total number of records = " + rowCount);
        } // end catch     
